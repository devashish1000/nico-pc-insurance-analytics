...
- A 4,184-row synthetic logical baseline was captured at `2026-07-13T23:13:04.347568Z` before warehouse-v2. Hosted migration history then recorded `warehouse_v2_foundation`, `warehouse_v2_loaders`, `warehouse_v2_orchestration_and_views`, `release_fk_indexes`, and `monotonic_composite_watermarks`; all 11 expected foreign-key supporting indexes are present. A post-run audit caught and repaired a future-seeded cursor regression while published fact counts/checksums and 6/6 DQ remained unchanged; 120 pgTAP assertions now cover the guard. This is deployment evidence, not a benchmark-performance, production-readiness, NICO approval, or UAT claim.
